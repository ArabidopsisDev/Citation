...
当所有的工作基本完成，需要导出所有引用格式化文本时，可以通过到处引用功能来完成。您有两种导出选项：一种是导出为格式文件，也就是Markdown和LaTeX格式等易于阅读的格式，另一种是交换文件，可以让他人通过您的交换文件将文献导入到自己的项目中，该过程无需网络。通过引用 > 导出引用 选择卡，软件将自动定位到导出引用页面。该页面包含项目中的所有引用信息，您可以通过上面的筛选菜单，通过课题名称、出版社等条件筛选您需要的论文。拖动论文可以对它们的导出顺序进行排序，点击向下的箭头可以展示论文详细信息。之后点击“导出引用”按钮，即可导出所有论文引用。
